...
from inspector_acts import load_model
...
def get_ig_scores(st_pairs, target_label, n_prompts_max, model_path, bs=1, ig_steps=5):
    ...
    return all_scores, all_scores_detection, all_scores_bsl, all_model_labels
...
def main():
    ...
    for label in [0, 1]:
        ...
        scores, scores_detection, scores_bsl, labels = get_ig_scores(
            st_pairs, 
            label, 
            args.texts, 
        ...
        print("save scores")
        output_path = os.path.join(args.data_path, f"scores_{label}_{args.dataset}_{args.exp_name}_rs_{args.rs}_acts.pkl")
        with open(output_path, 'wb') as f:
            pickle.dump(scores, f)

        print("save scores bsl")
        output_path = os.path.join(args.data_path, f"scores_bsl_{label}_{args.dataset}_{args.exp_name}_rs_{args.rs}_acts.pkl")
        with open(output_path, 'wb') as f:
            pickle.dump(scores_bsl, f)

        print("save scores detection")
        output_path = os.path.join(args.data_path, f"scores_detection_{label}_{args.dataset}_{args.exp_name}_rs_{args.rs}_acts.pkl")
        with open(output_path, 'wb') as f:
        ...
        # print(df.shape)
        # output_path = os.path.join(args.output_path, f"neuron_ranking_{label}_{args.dataset}_{args.exp_name}_rs_{args.rs}_acts.csv")
        ...
        # print(df_bsl.shape)
        # output_path = os.path.join(args.output_path, f"neuron_ranking_bsl_{label}_{args.dataset}_{args.exp_name}_rs_{args.rs}_acts.csv")
        ...
        # print(df_det.shape)
        # output_path = os.path.join(args.output_path, f"neuron_ranking_detection_{label}_{args.dataset}_{args.exp_name}_rs_{args.rs}_acts.csv")
