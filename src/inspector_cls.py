    ...
    def get_scores_for_layer(
        ...
        for token_idx in token_indexes:
            ...
            scaled_weights = self.scaled_input(
                baseline_activations, steps=ig_steps, device=self.device
            )  # dim = ig_steps x 3072 (intermediate layer size)
            ...
            integrated_grads_this_step *= baseline_activations.squeeze(0) / ig_steps
            integrated_grads.append(integrated_grads_this_step)
    
            integrated_grads = torch.stack(integrated_grads, dim=0).sum(dim=0) / len(
                integrated_grads
            )
            integrated_grads_all_toks.append(integrated_grads)
        integrated_grads_all_toks = torch.stack(integrated_grads_all_toks, dim=0)
        return integrated_grads_all_toks
    

def initialize_model_and_tokenizer(model_name: str, torch_dtype='auto', output_hidden_states=False):
    tokenizer = BertTokenizer.from_pretrained("bert-base-cased", clean_up_tokenization_spaces=False)
    model = AutoModelForSequenceClassification.from_pretrained(model_name, output_hidden_states=output_hidden_states)
    model.eval()
...
def load_model(model_name_or_path, device=None, output_hidden_states=False):
    model, tokenizer = initialize_model_and_tokenizer(model_name_or_path, output_hidden_states=output_hidden_states)
    if device is None:
